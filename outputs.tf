...
output "private_key_id" {
  ...
  value       = try(tls_private_key.this[0].id, null)
}

output "private_key_openssh" {
  description = "Private key data in OpenSSH PEM (RFC 4716) format"
  value       = try(trimspace(tls_private_key.this[0].private_key_openssh), null)
  sensitive   = true
}

output "private_key_pem" {
  description = "Private key data in PEM (RFC 1421) format"
  value       = try(trimspace(tls_private_key.this[0].private_key_pem), null)
  sensitive   = true
}
...
output "public_key_fingerprint_md5" {
  ...
  value       = try(tls_private_key.this[0].public_key_fingerprint_md5, null)
}
...
output "public_key_fingerprint_sha256" {
  ...
  value       = try(tls_private_key.this[0].public_key_fingerprint_sha256, null)
}
...
output "public_key_openssh" {
  ...
  value       = try(trimspace(tls_private_key.this[0].public_key_openssh), null)
}

output "public_key_pem" {
  description = "Public key data in PEM (RFC 1421) format"
  value       = try(trimspace(tls_private_key.this[0].public_key_pem), null)
}
